...
conditionalPanel("input.distribucion == 'Geometrica'", numericInput("proba_exitos_Geometrica", label = "Probabilidad de exito:", 0.5, min = 0, max = 1, step = 0.1))

conditionalPanel("input.distribucion == 'Binomial Negativa'", numericInput("p", label = "Probabilidad de exito:", 0.5, min = 0, max = 1, step = 0.1))
...
conditionalPanel("input.distribucion == 'Hipergeometrica'", numericInput("exitos_hiper", label = "Cantidad de exitos en la población:", 5, min = 1, max = 20, step = 1))

conditionalPanel("input.distribucion == 'Hipergeometrica'", numericInput("muestra_hiper", label = "Tamaño de la muesta que se extrae de  la población:", 5, min = 1, max = 20, step = 1))
...
renderPlot({
  ...
  geom_col(fill=wes_palette("Darjeeling1")[sample(1:5,1)]) +
  geom_text(
  ...
  datos <- data.frame(heads = intentos, 
           cdf = pbinom(q = intentos, size = 1, prob = input$proba_exitos_bernoulli, 
                        lower.tail = TRUE)) 
  datos %>%
  ggplot(aes(x = factor(heads), y = cdf)) +
  geom_col(fill=wes_palette("Darjeeling1")[sample(1:5,1)]) +
  geom_text(data = subset(datos, cdf>=0.01),
  ...
  datos <- data.frame(heads = intentos, 
           cdf = pbinom(q = intentos, size = (as.numeric(input$intentos)), prob = input$proba_exitos, 
                        lower.tail = TRUE))
  datos %>%
  ggplot(aes(x = factor(heads), y = cdf)) +
  geom_col(fill=wes_palette("Darjeeling1")[sample(1:5,1)]) +
...
datos%>%
  ...
  labs(title = "Función de Probabilidad Acumulada.",
    ...
    vector_probabilidad <- c()
    while(probabilidad<=0.99){
      probabilidad <- choose(k-1,as.numeric(input$r)-1)*(as.numeric(input$p)^as.numeric(input$r))*(1-as.numeric(input$p))^(k-as.numeric(input$r)) + probabilidad
      vector_probabilidad <- c(vector_probabilidad, probabilidad)
      k <- k+1
    }
    k <- k-1
...
datos %>%  ggplot(aes(x = factor(red), y = density)) +
  ...
  labs(title = "Probabilidad de X éxitos",
    ...
    datos <- data.frame(red = 1:as.numeric(input$muestra_hiper), density =probabilidad_Hipergeometrica(poblacion, exitos_poblacion, muestra))

    datos %>%  ggplot(aes(x = factor(red), y = density)) +
      geom_col(fill=wes_palette("Darjeeling1")[sample(1:5,1)]) +
      geom_text(data = subset(datos, density<=0.999),
        aes(label = round(density,2), y = density + 0.01),
        position = position_dodge(0.9),
        size = 3,
        vjust = 0
...
prob <- ppois(q = events, lambda = input$lambda, lower.tail = TRUE)
df <- data.frame(events, density, prob)
ggplot(df, aes(x = factor(events), y = prob)) +
  geom_col(fill = wes_palette("Darjeeling1")[sample(1:5,1)]) +
  geom_text(data = subset(df, prob<=0.999),
